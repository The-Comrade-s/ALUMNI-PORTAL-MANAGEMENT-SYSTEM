...
```env
NEXT_PUBLIC_API_URL=https://<your-koyeb-service>.koyeb.app
```
...
Preview at a staging Koyeb service if you have one.
...
2. Go to `/register` — the School/Department/Programme dropdowns should
   ...
   console — a CORS error there means `FRONTEND_URL` on the Koyeb backend
...
Once attached, update the Koyeb backend's `FRONTEND_URL` env var to match
...
cookie-based sessions — Vercel and Koyeb are unrelated domains, and
